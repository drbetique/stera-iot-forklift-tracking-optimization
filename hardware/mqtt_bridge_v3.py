...
# InfluxDB v3 Settings (High-frequency time-series data)
...
INFLUX_DATABASE = os.getenv("INFLUX_DATABASE", "forklift_data")
...
try:
    ...
    influx_enabled = True
    print(f"[OK] InfluxDB v3 client initialized")
except Exception as e:
    print(f"[WARN] InfluxDB initialization failed: {e}")
    print(f"[WARN] Running in MongoDB-only mode")
...
def write_to_influx(processed, forklift_id):
    """Store high-frequency data into InfluxDB v3."""
    if not influx_enabled:
        return

    try:
        # Create point for InfluxDB v3
        point = Point("mpu6050") \
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # 1. Write to InfluxDB v3 (high-frequency, all samples)
        ...
        # Console output
        influx_status = "[InfluxDB]" if influx_enabled else "[MongoDB-only]"
        print(f"{influx_status} {forklift_id}: accel={processed['accel_total_g']:.3f}g", end='')
        if aggregator.should_aggregate():
...
def main():
    print("=" * 60)
    print("Stera Forklift Dual-Database Bridge (InfluxDB v3)")
    print("MQTT → InfluxDB (high-frequency) + MongoDB (aggregated)")
    print("=" * 60)
    print(f"MQTT Broker:     {MQTT_BROKER}:{MQTT_PORT}")
    print(f"MQTT Topic:      {MQTT_TOPIC}")
    print(f"InfluxDB v3:     {INFLUX_HOST}")
    print(f"InfluxDB DB:     {INFLUX_DATABASE}")
    print(f"InfluxDB Status: {'Enabled' if influx_enabled else 'Disabled (MongoDB-only mode)'}")
